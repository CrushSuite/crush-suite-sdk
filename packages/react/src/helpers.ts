...
export const isAgeVerifiedForAlcohol = (
  customerDOB: string | Date | null,
  legalAge: number = LEGAL_AGE_FOR_ALCOHOL
): boolean => {
  if (!customerDOB) return false;

  const dob =
    typeof customerDOB === "string" ? new Date(customerDOB) : customerDOB;
